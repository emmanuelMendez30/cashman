...
// Al revés que `aISO`: una fecha "2026-09-15" de la base, como día local.
// `new Date("2026-09-15")` la lee como medianoche UTC, que en Costa Rica
// todavía es el 14 a las 6 p.m.
export function desdeISO(iso) {
  const [y, m, d] = iso.split("-").map(Number);
  return new Date(y, m - 1, d);
}

// "martes 15 de septiembre", para la rifa flash, que es de un solo día.
// Va en dos partes porque junto, el formato de "es" mete una coma después
// del día de la semana.
export function diaLegible(fecha) {
  const semana = fecha.toLocaleDateString("es", { weekday: "long" });
  const dia = fecha.toLocaleDateString("es", { day: "numeric", month: "long" });
  return `${semana} ${dia}`;
}

// "15 sept", para listas donde el día largo no entra.
