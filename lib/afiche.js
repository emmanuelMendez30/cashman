...
// El nombre que va arriba de todo. La rifa de la semana sale a nombre de
// Cashmana; la rifa flash, a nombre de Sr.Cash, así que viene por parámetro.
...
const COLUMNAS = 3;
// Lo que le queda al nombre después de los números. Da para un nombre
// completo de unos 26 caracteres, que es lo que miden los nombres largos de
// verdad ("Flor de María Obando", "Óscar Villalobos Camacho"). Lo que pase
// de ahí se recorta.
const ESPACIO_NOMBRE = 302;
const SEPARACION = 24;
const MARGEN = 48;
...
// `filas` es [{ numeros, nombre }] ya en el orden en que se quiere leer, una
// por persona: `numeros` son los que le tocaron, que en la rifa flash pueden
// ser dos. Se reparten por columna y no por renglón: la primera columna se
// llena entera de arriba abajo, después la segunda y después la tercera, que
// es como se busca un número en una lista pegada en la pared.
//
...
export function descargarAficheRifa(
  filas,
  { negocio = NEGOCIO, titulo = "RIFA DE LA SEMANA", rango, sorteo },
  nombreArchivo
) {
  // El ancho sale de cuántos números lleve cada uno: con dos, la columna se
  // ensancha en vez de comerle lugar al nombre, así que con uno solo la
  // medida termina siendo exactamente la de siempre.
  const porFila = Math.max(1, ...filas.map((f) => f.numeros.length));
  const SANGRIA_NOMBRE =
    porFila * CHIP_ANCHO + (porFila - 1) * CHIP_SEPARACION + 18;
  const ANCHO_COLUMNA = SANGRIA_NOMBRE + ESPACIO_NOMBRE;
  const ANCHO =
  ...
  // "CASHMANA", todo en mayúsculas, aguanta el espaciado ancho. Un nombre
  // con minúsculas como "Sr.Cash" se desarma con tanto aire entre letras,
  // así que va más junto.
  escribir(ctx, negocio, centro, 86, {
    fuente: `700 46px ${SANS}`,
    color: COLOR.texto,
    espaciado: negocio === negocio.toUpperCase() ? "8px" : "2px",
  });
  ...
  for (let fila = 0; fila < porColumna; fila++) {
    ...
    for (let columna = 0; columna < COLUMNAS; columna++) {
      ...
      item.numeros.forEach((numero, i) => {
        const xChip = x + i * (CHIP_ANCHO + CHIP_SEPARACION);
        ...
        escribir(ctx, numero, xChip + CHIP_ANCHO / 2, medio + 1, {
  ...
  // Cuando cada uno lleva más de un número, el total de la lista no es el de
  // números repartidos, y conviene que se lean los dos.
  const cuantosNumeros = filas.reduce((suma, f) => suma + f.numeros.length, 0);
  const pie =
    cuantosNumeros === filas.length
      ? `${filas.length} participantes`
      : `${filas.length} participantes · ${cuantosNumeros} números`;

  escribir(ctx, pie, centro, yPie + 66, {
